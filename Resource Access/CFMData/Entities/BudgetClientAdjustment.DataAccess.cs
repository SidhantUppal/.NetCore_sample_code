...
namespace CFMData
{
	public partial class BudgetClientAdjustment
	{
		...
		private BudgetClientAdjustment DataPortal_Fetch(BudgetClientAdjustmentCriteria criteria)
		{
			...
			{
				connection.Open();
				using (var command = new SqlCommand("[dbo].[spCFM_BudgetClientAdjustment_Select]", connection))
				{
                ...
                command.Parameters.AddWithValue("@p_UpdatedOnHasValue", criteria.UpdatedOnHasValue);
					...
					{
						var rowParser = reader.GetRowParser<BudgetClientAdjustment>();                       
						if(reader.Read())
						{
							return GetBudgetClientAdjustment(rowParser, reader);							
						}                            
						else
							throw new Exception(String.Format("The record was not found in 'dbo.BudgetClientAdjustment' using the following criteria: {0}.", criteria));
					}
				}
			}
			OnFetched();
       ...
       // [Transactional(TransactionalTypes.TransactionScope)]
		...
		protected   void UpdateChildren(BudgetClientAdjustment parent,SqlConnection connection,SqlTransaction trans)
		{
		...
		protected   void DataPortal_DeleteSelf()
		{
			...
			if (cancel) return;            
			DataPortal_Delete(new BudgetClientAdjustmentCriteria (BudgetClientAdjustmentID));        
			OnSelfDeleted();
		}
        
		//[Transactional(TransactionalTypes.TransactionScope)]
		protected void DataPortal_Delete(BudgetClientAdjustmentCriteria criteria)
		{
            bool cancel = false;
            ...
            {
                connection.Open();
                using (var command = new SqlCommand("[dbo].[spCFM_BudgetClientAdjustment_Delete]", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
        ...
        ///    CSLA editable child business object of type <see cref="BudgetClientAdjustment"/> 
        /// </summary>
        /// <returns></returns>
        ...
        {
            Child_Insert(budget, null, null, connection,trans);
        }


        public void Child_Insert(BudgetLineCategory budgetLineCategory, SqlConnection connection,SqlTransaction trans)
        {
            Child_Insert(null, budgetLineCategory, null, connection,trans);
        }


        public void Child_Insert(HomeClient homeClient, SqlConnection connection,SqlTransaction trans)
        {
            Child_Insert(null, null, homeClient, connection,trans);
        }
        ...
        {
            Child_Update(budget, null, null, connection,trans);
        }
        ...
        private void Child_DeleteSelf(SqlConnection connection)
        {
            ...
            //DataPortal_Delete(new BudgetClientAdjustmentCriteria (BudgetClientAdjustmentID), connection);
		...
		public void InitDTO()
		{
			  BudgetClientAdjustmentDTO dt=new BudgetClientAdjustmentDTO();
			dt.BudgetClientAdjustmentID =this.BudgetClientAdjustmentID ;
			dt.BudgetID =this.BudgetID ;
			dt.CategoryID =this.CategoryID ;
			dt.HomeClientID =this.HomeClientID ;
			dt.AdjustmentAmount =this.AdjustmentAmount ;
  ...
  this.CurrentDTO = dt;
        ...
        private void Map(SafeDataReader reader)
        {
            ...
            {
                LoadProperty(_budgetClientAdjustmentIDProperty, reader["BudgetClientAdjustmentID"]);
                LoadProperty(_budgetIDProperty, reader["BudgetID"]);
                LoadProperty(_categoryIDProperty, reader["CategoryID"]);
                LoadProperty(_homeClientIDProperty, reader["HomeClientID"]);
                LoadProperty(_adjustmentAmountProperty, reader["AdjustmentAmount"]);
