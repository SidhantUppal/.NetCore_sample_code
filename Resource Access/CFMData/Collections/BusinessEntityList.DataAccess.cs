...
namespace CFMData
{
    public partial class BusinessEntityList
    {
        
        private BusinessEntityList DataPortal_Fetch(BusinessEntityCriteria criteria)
        {
            bool cancel = false;
            ...
            using (var connection = new SqlConnection(ADOHelper.ConnectionString))
            {
                ...
                using (var command = new SqlCommand("[dbo].[spCFM_BusinessEntity_Select]", connection))
                {
                    ...
                    using(var reader = command.ExecuteReader())
                    {
                        ...
                        if(reader.Read())
                        {
                           var rowParser = reader.GetRowParser<CFMData.BusinessEntity>();
                           do
                           {
								
                              this.Add(CFMData.BusinessEntity.GetBusinessEntity(rowParser, reader));
                           }while(reader.Read());
                        }
						OnFetched();
                        return this;
