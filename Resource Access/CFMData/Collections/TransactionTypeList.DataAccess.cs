...
namespace CFMData
{
    public partial class TransactionTypeList
    {
        ...
        private TransactionTypeList DataPortal_Fetch(TransactionTypeCriteria criteria)
        {
            bool cancel = false;
            ...
            using (var connection = new SqlConnection(ADOHelper.ConnectionString))
            {
                ...
                command.Parameters.AddWithValue("@p_UpdatedOnHasValue", criteria.UpdatedOnHasValue);
                    using(var reader = command.ExecuteReader())
                    {
                        ...
                        if(reader.Read())
                        {
                           var rowParser = reader.GetRowParser<CFMData.TransactionType>();
                           do
                           {
								
                              this.Add(CFMData.TransactionType.GetTransactionType(rowParser, reader));
                           }while(reader.Read());
                        }
						OnFetched();
                        return this;
